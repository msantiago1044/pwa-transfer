...
// Extracts session code from either a plain code or a full URL (?join=CODE)
...
export default function SessionJoin({ onSessionJoined }: SessionJoinProps) {
  ...
  function handleManualInput() {
    ...
    if (!trimmedInput) { setError('Por favor ingresa un codigo'); return; }
    if (!isValidSessionId(trimmedInput)) { setError('Codigo invalido. Debe tener 8 caracteres'); return; }
    setError(null);
    onSessionJoined(trimmedInput);
  }

  function handleScanSuccess(raw: string) {
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={manualInput}
                  onChange={(e) => { setManualInput(e.target.value.toUpperCase()); setError(null); }}
                  onKeyPress={(e) => { if (e.key === 'Enter') handleManualInput(); }}
              ...
              <button onClick={handleManualInput} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 rounded-xl">
                Conectar
              </button>
              <button
